...
export async function createteacher(staff_id: any, last_name: string, first_name: string = "") {
...
export async function importFromCSV(csvRows: any[]) {
    for (const row of csvRows) {
        ...
        console.log(`${cte_school.id} ${cte_school.name}`)
        ...
        //the reason only one school is populating is because
        //the same class is in two schools, probalem to sort out later.
        if (!course_instance) {
            course_instance = await Course_Instance.create({
                cte_school_id: cte_school.id,
                course_catalog_id: course?.id,
                alias: program?.title,
                district_program_id: cte_program.id,
