...
export async function generateStudentSkillsPdf(req: Request, res: Response) {
    try {
        ...
        const [skills, wblRecords, rubricSection, staticFields, cdosFields] = await Promise.all([
...
function safeSetText(form: PDFForm, fieldName: string, value: string | number | null | undefined) {
    try {
...
function fillCDOS(form: PDFForm, section: any) {
    ...
    rows.forEach((row: any, rowIndex: number) => {
        ...
        columns.forEach((col: any, colIndex: number) => {
            const grade = row.rubric_grades?.find((g: any) => g.rubric_column_id === col.id)

            const periodSuffixes = ["junior_sem1_", "junior_sem2_", "senior_sem1_", "senior_sem2_"]
...
function fillLegacySkills(form: PDFForm, skills: any[]) {
    ...
    for (let i = 0; i < 17; i++) {
        ...
        if (skillItem) {
            ...
            safeSetText(form, `Ind_Tech_Expectation_.${indexSuffix}`, skillItem.description)

            const scores = Array.isArray(skillItem.SkillScores) ? skillItem.SkillScores : []
...
function fillWBLData(form: PDFForm, wblRecords: any[]) {
    ...
    const wblSummaries = wblRecords.map((cat: any) => ({
        name: cat.name,
        totalHours: cat.wbl_hours.reduce((sum: number, hour: any) => sum + (hour.hours || 0), 0),
    }))
