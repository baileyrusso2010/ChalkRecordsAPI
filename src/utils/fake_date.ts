...
import { Attendance } from "../models/attendance/attendance_daily.model";
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    for (let j = 0; j < 30; j++) {
      await Attendance.create({
        ...
        enrollment_id:
          enrollments[Math.floor(Math.random() * enrollments.length)].id, //kind of bad but it works
  ...
  for (let i = 0; i < students.length; i += 3) {
    ...
    for (let j = 0; j < 3; j++) {
      await Behavior.create({
        ...
        behavior_type_id:
          all_types[Math.floor(Math.random() * all_types.length)].id,
