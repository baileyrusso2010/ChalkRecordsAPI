...
import { Rubric_Criteria } from "../../models/rubric/rubric_criteria.model";
...
export async function createRubricEntry(req: Request, res: Response) {
  try {
    ...
    const entry = await Rubric_Criteria.create({
      rubric_id,
      name,
      description,
      score,
    ...
    if (!entry)
      return res.status(400).json({ error: "Failed to create rubric entry" });
