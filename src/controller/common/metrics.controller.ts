...
function getAttributes(metric: string, level: string) {
  ...
  if (metric === "behavior") {
    ...
    attributes.push([
      Sequelize.fn("COUNT", Sequelize.col("Behavior.id")),
      "incident_count",
    ]);
    return attributes;
  }
  ...
  if (metric === "attendance") {
    ...
    attributes.push([
      Sequelize.fn("AVG", Sequelize.col("percent_score")),
      "value",
    ]);
  }
...
export async function getAttendanceMetrics(req: Request, res: Response) {
  try {
    ...
    switch (level) {
      ...
      default:
        return res.status(400).json({
          error: "Invalid level. Must be 'school', 'grade', or 'student'",
        });
    }

    // Add attendance metrics to attributes
    attributes.push([
      Sequelize.fn("COUNT", Sequelize.col("Attendance.id")),
      "total_records",
    ]);
    attributes.push([
      Sequelize.literal(
        "SUM(CASE WHEN \"Attendance\".code = 'P' THEN 1 ELSE 0 END)"
      ),
      "present_count",
    ]);
    attributes.push([
      Sequelize.literal(
        "SUM(CASE WHEN \"Attendance\".code = 'A' THEN 1 ELSE 0 END)"
      ),
      "absent_count",
    ]);
    attributes.push([
      Sequelize.literal(
        "SUM(CASE WHEN \"Attendance\".code = 'T' THEN 1 ELSE 0 END)"
      ),
      "tardy_count",
    ]);
    attributes.push([
    ...
    const results = await Attendance_Daily.findAll({
      attributes,
